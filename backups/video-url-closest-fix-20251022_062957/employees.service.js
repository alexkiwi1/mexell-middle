      ...
      employee.sessions = workSessions.map((session, index) => {
        ...
        if (session.cameras && session.cameras.length > 0) {
          ...
          if (employeeEvents.length > 0) {
            ...
            // 2. Fallback: find the closest event within 5 minutes before or after
            if (!matchingEvent) {
              ...
              if (eventsNearby.length > 0) {
                ...
                matchingEvent = eventsNearby.reduce((closest, event) =>
                  Math.abs(event.start_time - session.start_time) < Math.abs(closest.start_time - session.start_time)
                    ? event : closest
                );
              }
            }
